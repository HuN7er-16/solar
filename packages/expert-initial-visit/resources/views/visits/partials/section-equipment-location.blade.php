    ...
    <div class="card-body p-4">
        <div class="row g-4">
            ...
            <div class="col-md-4">
                ...
                <div class="d-flex gap-4 p-3 @error('equipment_ventilation_ok') border border-danger @enderror" style="background:#F5F5F5;border-radius:8px;">
                ...
                @error('equipment_ventilation_ok')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
            </div>

            <div class="col-md-4">
                <label class="form-label fw-semibold mb-2">مسیر مناسب کابل‌کشی: <span class="text-danger">*</span></label>
                <div class="d-flex gap-4 p-3 @error('cable_route_ok') border border-danger @enderror" style="background:#F5F5F5;border-radius:8px;">
                    @foreach(['1'=>'بله','0'=>'خیر'] as $v=>$l)
                    <div class="form-check">
                ...
                @error('cable_route_ok')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
            </div>

            <div class="col-md-4">
                <label class="form-label fw-semibold mb-2">نیاز به ایجاد فضای جدید: <span class="text-danger">*</span></label>
                <div class="d-flex gap-4 p-3 @error('new_equipment_space_needed') border border-danger @enderror" style="background:#F5F5F5;border-radius:8px;">
                    @foreach(['1'=>'بله','0'=>'خیر'] as $v=>$l)
                    <div class="form-check">
                ...
                @error('new_equipment_space_needed')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
            </div>

            <div class="col-12">
                <label class="form-label fw-semibold mb-2">توضیحات:</label>
                <textarea name="equipment_location_notes" rows="2" class="form-control"
                          style="border-radius:8px;">{{ old('equipment_location_notes') }}</textarea>
            </div>
